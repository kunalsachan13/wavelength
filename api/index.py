...
# Add local directory and SpotAPI clone to Python path
cur_dir = os.path.dirname(__file__)
if cur_dir not in sys.path:
    sys.path.insert(0, cur_dir)

spotapi_path = os.path.abspath(os.path.join(cur_dir, "..", "..", "SpotAPI"))
if os.path.exists(spotapi_path) and spotapi_path not in sys.path:
...
try:
    ...
    _spotapi_error = f"{type(e).__name__}: {str(e)}"
...
for r in list(app.routes):
    if hasattr(r, "endpoint") and hasattr(r, "methods") and hasattr(r, "path"):
        if not r.path.startswith("/api") and r.path not in ("/openapi.json", "/docs", "/redoc"):
            app.add_api_route(
                f"/api/music{r.path}",
                r.endpoint,
                methods=list(r.methods),
                name=f"api_music_{r.name}",
            )
            app.add_api_route(
                f"/api{r.path}",
                r.endpoint,
